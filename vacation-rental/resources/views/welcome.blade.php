        ...
        @if (Route::has('login'))
            <nav class="flex items-center justify-end gap-4">
                @auth
                    ...
                    <a href="{{ url('/home') }}" class="inline-block px-5 py-1.5 border text-sm rounded-sm">
                        Dashboard
                    </a>

                    <!-- Or create a simple test link -->
                    <a href="{{ route('home') }}" class="inline-block px-5 py-1.5 border text-sm rounded-sm">
                        Home
                    </a>

                    <form method="POST" action="{{ route('logout') }}" class="inline-block">
                        @csrf
                        <button type="submit" class="btn btn-link text-primary text-decoration-underline bg-white border-0"
                            style="text-decoration: underline;">
                            Logout
                        </button>
                    </form>
                @else
                    <a href="{{ route('login') }}" class="inline-block px-5 py-1.5 border text-sm rounded-sm">
                        Log in
                    </a>

                    @if (Route::has('register'))
                        <a href="{{ route('register') }}" class="inline-block px-5 py-1.5 border text-sm rounded-sm">
                            Register
                        </a>
                    @endif
                @endauth
